...
C'est le même argument que vous avez tendance à entendre pour
l'apprentissage du Latin. Ça ne vous donnera pas de travail, sauf peut-être
en tant que professeur de lettres classiques, mais ça améliora votre esprit
et fera de vous un meilleur écrivain dans les langues que vous voulez utiliser,
comme l'anglais.
...
est un langage de programmation, et les ordinateurs parlent tous les langages
...
faire de lui un meilleur peintre, il me semble qu'il voudrait l'utiliser dans
toutes ses peintures, n'est-ce pas ? Je n'essaie pas de me moquer d'Eric
Raymond. Dans l'ensemble, ses conseils sont bons. Ce qu'il dit de Lisp est
plutôt du bon sens. Mais il y a une contradiction dans le bon sens :
Lisp ferra de vous un meilleur programmeur et
...
une activité très compétitive, sujette aux monopoles naturels. Une entreprise
...
de ne pas faire confiance à notre instinct et ne pas utiliser Lisp. On
...
moyenne le fait, vous pouvez vous attendre à faire aussi bien que la grande
...
tout comme une startup moyenne, vous pouvez anticiper une performance moyenne. Le
problème est que la performance moyenne signifie que vous ferez faillite. Le
taux de survie des startups est beaucoup moins de cinquante pourcent. Ça
veut dire que si vous dirigez une startup, vous feriez mieux de faire
...
comprenaient, et que peu comprennent même maintenant : quand vous écrivez
un logiciel qui ne va fonctionner que sur vos propres serveurs, vous pouvez
utiliser un langage de programmation de votre choix. Quand vous écrivez
un logiciel de bureau, il y a un fort biais pour l'écrire dans le même langage
...
serait important sur ce marché. Nous partions tous de zéro, ainsi, une
...
un logiciel la minute où il est fait.
...
qu'on pouvait obtenir. Quand on a lancé Viaweb, on n'avait aucune expérience en
affaires. On ne savait rien à propos de marketing, d'embauche, de collecte
de fonds, ou d'obtention de clients. Aucun de nous n'avait même jamais eu ce
que vous appelleriez un vrai travail. La seule chose dans laquelle nous
étions bons était l'écriture de logiciels. On espérait que ça nous
sauverait. Nous prendrions tout avantage que nous pourrions obtenir dans le
département du logiciel.
...
qu'ils ne pouvaient pas faire. Et puisque Lisp est de si haut niveau, nous
n'aurions pas besoin d'une grande équipe de développement, donc nos coûts
seraient plus bas. Si c'était le cas, on pourrait offrir un meilleur
...
vingt à trente, mais aucuns de leurs logiciels ne pouvait rivaliser
avec le nôtre. Nous avions un WYSIWYG constructeur de magasin en ligne qui
fonctionnait sur le serveur et pourtant se comportait comme un application de
...
Il a dû sembler à nos concurrents que nous avions une sorte d'arme secrète
— que nous décodions leur trafic Enigma ou quelque chose du genre. En effet,
on avait une arme secrète, mais elle était plus simple qu'ils ne le pensaient.
...
Jackal* de Frederick Forsyth. Le personnage principal est un assassin qui
est engagé pour tuer le président de la France. L'assassin doit passer
devant la police pour se hisser jusqu'à un appartement qui surplombe la route du
...
des années, cela m'énervait d'entendre Lisp décrit comme ça.
Mais après tout, cela a fonctionné à notre avantage. En affaires, rien ne
...
n'était pas un accident. Une startup doit donner à ses concurrents le moins
...
avons eu plus d'utilisateurs. À la fin de 1996, nous avions environ 70
magasins en ligne. À la fin de 1997, nous en avions 500. Six mois après, quand
...
combien d'utilisateurs ils ont en ce moment, mais la dernière fois que j'en ai
entendu parler, il y en avait environ 20 000.

## Le paradoxe de Blub

En quoi Lisp est-il si génial ? Et si Lisp est si génial, pourquoi tout le
monde ne l'utilise pas ? Ces questions-ci ont l'air rhétorique, mais en
...
langages de programmation ne sont pas simplement des technologies, mais des
...
n'importe quel langage sauf le plus puissant[3](footnote:3).
...
écrit en un certain langage, ça peut être une bonne idée d'écrire le
nouveau programme dans le même langage. Si vous écrivez un programme qui doit
faire quelque chose de simple, comme du calcul numérique ou la
...
sont souvent tous traités comme équivalents. Ils ne le sont pas. Techniquement,
...
plus proche du langage machine que Python.
...
le considérer comme juste assez bon.
...
quoi que ce soit avec ça. Cobol n'a même pas X (fonctionnalité de Blub de votre choix).
...
puissance, il ne se rend pas compte de ce qu'il regarde. Ce qu'il voit ne sont
...
constatons qu'il à son tour, méprise Blub. Comment on peut arriver à quoi
que ce soit avec Blub ? Il n'a même pas Y.
...
mais ça ne m'a pas manqué à l'époque. Je pensait en Basic. Et j'étais un
...
langages. Je me dis : comment on peut arriver à quoi que ce soit avec eux,
sans macros[5](footnote:5) ?

Plusieurs langages ont quelques choses qui s'appellent une « macro ». Mais
les macro de Lisp sont uniques. Et croyez-le ou non, ce qu'elles font est lié
...
arriver aux macros avant la page 160.
...
d'environ 20 à 25% de macros. Les macros sont plus difficiles à écrire que les
fonctions ordinaires, et il est considéré une mauvaise pratique de les utiliser
quand elles ne sont pas nécessaires. Donc, chaque macro dans ce code est là
car elle doit y être. Cela signifie qu'au moins 20 à 25% du code de ce
programme fait des choses que vous ne pouvez pas facilement faire dans un
...
parce qu'il n'est pas largement utilisé. Dans une situation
...
donc le langage médian, c'est-à-dire le langage utilisé par le
programmeur médian, quel qu'il soit, se déplace aussi lentement qu'un iceberg.

Les ramasse-miettes, introduite par Lisp vers 1960 sont maintenant largement
considéré comme une bonne chose. Le typage au runtime, de même, gagne en popularité.
Les fermetures lexicales, introduites par Lisp au début des années 1970,
sont maintenant, à peine, sur l'écran radar. Les macros, introduites par Lisp
...
puissiez combattre cette force puissante. Ce que je propose est exactement le
...
concurrents, collés immobiles au langage médian, ne pourront jamais égaler.
...
chaque mois environ. La première chose que je faisais, après avoir vérifié
s'ils avaient une démo en ligne en direct, était de consulter leurs listes
d'emploi. Après quelques années, je pouvais savoir de quelles entreprises
se soucier et desquelles ne pas se soucier. Plus les descriptions de
poste avaient une saveur informatique, mois l'entreprise était dangereuse.
Les types les plus innofensifs étaient ceux qui voulaient une expérience Oracle.
...
> footnotes
  ...
  1. Au début, Viaweb comportait deux parties : l'éditeur, écrit en Lisp,
     ...
     était surtout en Lisp, parce que le système de commandes était petit. Plus
  ...
  3. Tous les langages sont également puissants dans le sens où ils sont
     Turing-équivalants, mais ce n'est pas le sens du mot auquel les
     ...
     être formellement définissable, mais une façon de l'expliquer serait
     de dire qu'il réfère à des fonctionnalités que vous ne pouvez obtenir
     dans le langage moins puissant qu'en écrivant un interpréteur pour le
     langage plus puissant. Si le langage A a un opérateur pour la suppression
     ...
     sous-programme pour le faire en B. Mais si A supporte, disons,
     récursivité et B ne la supporte pas, ce n'est pas probable que quelqu'un
     puisse le corriger en écrivant des fonctions de bibliothèque.

  4. Note aux nerds : ou peut-être que le treillis se rétrécit vers le haut
  ...
  5. C'est un peu déroutant de traiter les macros comme une fonctionnalité
     ...
     autres caractéristique de Lisp comme les fermetures lexicales et les
     paramètres rest (???).
