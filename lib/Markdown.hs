  ...
  case MMark.parse path txt of
    ...
    Right doc -> do
      let headings = MMark.runScanner Ext.headingScanner doc
          ...
          toc = MMark.runScanner (Ext.tocScanner (\x -> x > 1 && x < 5)) doc
          transformations =
            Ext.checkFragments headerIds
              >=> Ext.checkIconsWith [scheme|fa|] icons
              >=> Ext.checkAltText
...
renderExtensions =
  mconcat
    [ Ext.iconsWith [scheme|fa|] "icon" icons,
      Ext.footnotes,
      Ext.kbd,
      addLinkRel,
      Ext.linkTarget,
...
disableBullets :: RenderExtension
disableBullets = Render.blockRender $ \old block ->
  case block of
    UnorderedList _ (item :| [])
      | (marker : UnorderedList spn sub : _) <- item,
        isMarker marker ->
          L.with (old (UnorderedList spn sub)) [L.class_ "no-bullets"]
    other -> old other
  where
    isMarker blk = case blk of
      Naked _ (ois, _) -> plain ois == noBulletsMarker
      Paragraph _ (ois, _) -> plain ois == noBulletsMarker
      _ -> False
    plain = Render.asPlainText . getOis
...
-- | The icons that @\<fa:name\>@ autolinks stand for. This site does not load
-- the Font Awesome webfont, so each name maps to a self-contained SVG that is
-- put in the page. 'Ext.checkIcons' turns a name that is not here into an
-- error at the markup that names it, so an icon cannot go missing quietly.
--
-- The SVGs mirror the templates/icon-*.mustache partials.
icons :: Map Text (L.Html ())
icons =
  L.toHtmlRaw
    <$> Map.fromList
